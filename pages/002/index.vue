	...
	**********************************************************/
	...
	const setChkList = (idx) => {
		...
		if (iCheck == 1) {
			...
			if (isChkAllMode == -1) isChkAllMode = 0
			else {
				...
				if (iCount == liwaData.value.length) isChkAllMode = 1			
			}
		} else {
			// unChecked
			...
			if (liwaData.value.length > iCount) isChkAllMode = 0
			else isChkAllMode = -1
		}
	}
...
<div class="w-full bg-slate-200 px-4 py-2">
	<div class="barPanel h-12 rounded-3xl ml-4 mb-2 px-1">
		...
		<div class="top-icon ml-2 -mt-1" @click="">
